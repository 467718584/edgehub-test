...
async function loadTransfers() {
  ...
  try {
    ...
    const resp = await fetch(apiBase + '/transfers', {
      headers: { 
    ...
    if (!resp.ok || !contentType || !contentType.includes('application/json')) {
      ...
      throw new Error('API返回格式错误: ' + resp.status);
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (pushForm) {
    pushForm.addEventListener('submit', async function(e) {
      ...
      try {
        const apiBase = getApiBase();
        const resp = await fetch(apiBase + '/transfers/push', {
          method: 'POST',
          headers: {
  ...
  if (pullForm) {
    pullForm.addEventListener('submit', async function(e) {
      ...
      try {
        const apiBase = getApiBase();
        const resp = await fetch(apiBase + '/transfers/pull', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-API-Key': 'edgehub_secret_key'
